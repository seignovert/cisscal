Pro Cisscal_Rdpix, Image,X0, Y0, scale=scale	
...
;		assumed to be zero.
;
;       scale:  Factor by which to multiply X and Y values for
;               display; useful for large images that don't fit in
;               display window without scaling
;
; OUTPUTS:
;	None.
;
...
;       
;       Ben Knowles (knowles@ciclops.org), 3/7/05;
;               Minor change for CISSCAL so that correct image pixel
;               values are displayed for large images
;-
...
if keyword_set(scale) then scl=scale else scl=1.0
...
while !mouse.button ne 4 do begin
	CURSOR,x,y,2,/dev
        ...
        if (x le s[1]) and (y le s[2]) and (x ge 0) and (y ge 0) then begin
            ...
            print,form = form, x*scl,y*scl,Image[x,yy],cr
        endif
endwhile
print,form="(/)"
end
